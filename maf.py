...
parser.add_argument('--kl', action='store_true', help='Compute empirical kl divergence.')
parser.add_argument('--restore_file', type=str, help='Path to model to restore.')
parser.add_argument('--restore_file_compare', type=str, default=None, help='Path to model to compare (q in kl(p, q)) to restore.')
...
parser.add_argument('--eval_epochs', type=int, default=1, help='How often to evaluate')
...
def evaluate(model, dataloader, epoch, args):
    ...
    if args.cond_label_size is not None:
        ...
        if args.dataset in ["TEMPERATURE0", "TEMPERATURE1", "TEMPERATURE2"]:
            loglike = []
            for x, y in dataloader:
                x = x.view(x.shape[0], -1).to(args.device)
                loglike.append(model.log_prob(x, y))
            loglike = torch.cat(loglike, dim=0)
            logprobs = loglike # we only consider conditional probability
        else:
...
def train_and_evaluate(model, train_loader, test_loader, optimizer, args):
    ...
    for i in range(args.start_epoch, args.start_epoch + args.n_epochs):
        ...
        print("eval")
        if (i + 1) % args.eval_epochs == 0 or i + 1 == args.n_epochs:
...
def plot_density(dist, ax, ranges, flip_toy_var_order=False):
...
if __name__ == '__main__':
    ...
    # load data
    if args.conditional: assert args.dataset in ['MNIST', 'CIFAR10', 'TEMPERATURE0', "TEMPERATURE1", "TEMPERATURE2"], \
            'Conditional inputs only available for labeled datasets MNIST, CIFAR10 and TEMPERATURE.'
    ...
    if args.kl:
        ...
        assert args.restore_file_compare, "restore_file_compare is required if kl is set"
        if args.restore_file:
            state = torch.load(args.restore_file_compare, map_location=args.device)
            model_q.load_state_dict(state['model_state'])
        ...
        for data in train_dataloader:
            ...
            log_p = model.log_prob(x, y if args.cond_label_size else None).sum(0).item()
            log_q = model_q.log_prob(x, y if args.cond_label_size else None).sum(0).item()
            log_p_minus_q.append(log_p - log_q)
        print("kl divergence = {}".format(sum(log_p_minus_q) / count))
